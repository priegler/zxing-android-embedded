...
import at.nineyards.qrcodescanner.Contents;
import at.nineyards.qrcodescanner.LocaleManager;
import at.nineyards.qrcodescanner.PreferencesActivity;
import at.nineyards.qrcodescanner.R;
import at.nineyards.qrcodescanner.book.SearchBookContentsActivity;
...
public abstract class ResultHandler {
    ...
    /**
     * The icon of the nth action button.
     *
     * @param index From 0 to getButtonCount() - 1
     * @return The button text as a resource ID
     */
    public abstract int getButtonIcon(int index);
    ...
    private static int doToContractType(String typeString, String[] types, int[] values) {
        ...
        for (int i = 0; i < types.length; i++) {
            String type = types[i];
            if (typeString.startsWith(type) || typeString.startsWith(
    ...
    final void openProductSearch(String upc) {
        Uri uri = Uri.parse(
                "http://www.google." + LocaleManager.getProductSearchCountryTLD(activity) +
    ...
    final void openBookSearch(String isbn) {
        Uri uri = Uri.parse(
                "http://books.google." + LocaleManager.getBookSearchCountryTLD(activity) +
                        "/books?vid=isbn" + isbn);
        launchIntent(new Intent(Intent.ACTION_VIEW, uri));
    }
